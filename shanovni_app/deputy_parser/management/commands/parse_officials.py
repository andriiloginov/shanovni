...
class Command(BaseCommand):
    help = 'Parse deputy data from a given URL and update Official model'
    ...
    def handle(self, *args, **options):
        ...
        if deputy_data:
            if not deputy_data.get('name'):
                logging.error(f"Не знайдено ім'я для {url}. Не зберігаю запис. HTML snippet: {deputy_data.get('html_snippet', 'N/A')}")
            ...
            try:
                ...
                if created:
                    self.stdout.write(self.style.SUCCESS(f"Створено новий запис для {deputy_data.get('name', 'NO NAME')} з slug: {deputy_data.get('slug')}"))
                else:
                    self.stdout.write(self.style.SUCCESS(f"Оновлено дані для {deputy_data.get('name', 'NO NAME')} з slug: {deputy_data.get('slug')}"))
